function average = nanmean2(x, value)
	% Because images won't allow NaN values...
	TotalSum = sum(sum(x));
	XSize = size(x);
	TotalNum = XSize(1)*XSize(2);
	IgnoredNum = sum(sum(x == value));
	NetNum = TotalNum - IgnoredNum;
	NetSum = TotalSum - value*IgnoredNum;
	average = NetSum/NetNum;
end
